// @ts-nocheck
/* ============================================================================
   LightPillar — ported from React Bits (reactbits.dev/backgrounds/light-pillar),
   a three.js ray-marched vertical "pillar of light". Recolored to EVA's dark-blue
   / metallic palette at the call site via topColor/bottomColor/intensity.

   Two safety changes vs the upstream copy:
   • `tanh()` is GLSL ES 3.00 only — replaced with a WebGL1-safe `tanhv()` helper
     so the shader compiles under ANGLE (Chrome/Windows prod).
   • disposes the renderer + forces context loss on unmount (our WebGL-leak rule).
   ========================================================================== */
...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      // WebGL1-safe tanh (GLSL ES 1.00 has no tanh); abs+sign avoids exp overflow.
      vec3 tanhv(vec3 x) {
      ...
      void main() {
        ...
        color = tanhv(color * uGlowAmount / widthNormalization);
