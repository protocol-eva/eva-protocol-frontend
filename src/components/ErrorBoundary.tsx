...
function DefaultFallback({
  ...
  return (
    ...
    >
      <div className="gl-onyx-panel max-w-[520px] w-full rounded-2xl px-7 py-7">
        <div className="text-[13px] font-extrabold uppercase tracking-[0.18em] gl-metal-text-blue">
        ...
        <pre
          className="mt-3.5 rounded-[10px] px-3 py-2.5 text-xs whitespace-pre-wrap break-words max-h-[140px] overflow-auto font-mono"
          style={{
        ...
        <div className="mt-4 flex flex-col sm:flex-row gap-2.5">
          <button
            type="button"
            onClick={reset}
            className="gl-navbar-btn flex-1 rounded-xl py-2.5 px-4 text-sm font-bold cursor-pointer"
          >
            Retry
          </button>
          <button
            type="button"
            onClick={goHome}
            className="flex-1 rounded-xl py-2.5 px-4 text-sm font-bold cursor-pointer transition-colors"
            style={{
              border: '1px solid var(--panel-border)',
              background: 'var(--surface-secondary)',
              color: 'var(--text-primary)',
