...
export default function BrandStats() {
  return (
    <section className="bg-eva-accent py-20 relative overflow-hidden">
      {/* Halftone Pattern */}
      <div
      ...
      <div className="max-w-[1920px] mx-auto px-4 lg:px-16 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-12 md:text-left">
          {stats.map((stat, i) => (
            ...
            >
              {/* Mobile Neon Corners */}
              <div className="absolute top-0 right-0 w-3 h-3 border-t-2 border-r-2 border-eva-gold md:hidden opacity-80 shadow-[0_0_10px_rgba(234,179,8,0.5)]"></div>
              <div className="absolute bottom-0 left-0 w-3 h-3 border-b-2 border-l-2 border-eva-gold md:hidden opacity-80 shadow-[0_0_10px_rgba(234,179,8,0.5)]"></div>

              {/* Mobile Inner Glow */}
              <div className="absolute inset-0 bg-eva-gold/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none md:hidden"></div>
