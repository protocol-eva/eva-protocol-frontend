...
export function DataPage() {
  useEffect(() => {
    ...
    const FUT_TTL = 25_000
    let futSym = 'BTC'
    let futTab = 'liqmap'
    let futData: unknown = null
    const futInflight = new Map<string, Promise<unknown | null>>()
    const lastLiqMarkBySym: Record<string, number> = {}
    ...
    async function fetchFutData(sym: string, force = false) {
      const cacheKey = ['futures', sym].join(':')
      if (force) _cache.delete(cacheKey)

      const inflight = futInflight.get(sym)
      ...
      const request = (async () => {
        try {
          ...
          console.error('futData:', e)
          return null
        } finally {
          futInflight.delete(sym)
        }
      })()
    ...
    function renderFutPanels(data: unknown) {
      if (!data) return
      futData = data
      if (futTab === 'liqmap') renderLiqMap(data)
      else renderInflow(data)
      renderAISignals(data)
    }

    function switchFutSym(sym: string) {
      if (sym === futSym) return
      futSym = sym
      const pillsEl = el('fut-sym-pills')
      if (pillsEl) {
        pillsEl.querySelectorAll('.sym-pill').forEach((b) => {
          b.classList.toggle('active', (b as HTMLElement).dataset.sym === sym)
        })
      }

      const cached = _cache.get(['futures', sym].join(':'))
    ...
    function prefetchFutSymbols() {
      FUT_SYMS.forEach((sym) => {
        void fetchFutData(sym)
      })
    }

    function liqMarkerInsertIdx(mark: number, reversed: { price: number }[]) {
      if (!reversed.length) return 0
      if (mark >= reversed[0].price) return 0
      if (mark <= reversed[reversed.length - 1].price) return reversed.length
      for (let i = 0; i < reversed.length - 1; i++) {
        const high = reversed[i].price
        const low = reversed[i + 1].price
        if (mark <= high && mark >= low) {
          const span = high - low || 1
          const frac = (high - mark) / span
          return frac >= 0.5 ? i + 1 : i
        }
      }
      const idx = reversed.findIndex((l) => l.price <= mark)
      return idx < 0 ? reversed.length : idx
    }

    function liqMarkerRowHtml(mark: number, idx: number) {
      return `<div id="liq-price-marker-row" class="flex items-center gap-1.5 shrink-0 liq-price-marker-row" data-idx="${idx}"><span class="liq-price-marker__price">${fmtPx(mark)}</span><div class="liq-price-marker__track"><span class="liq-price-marker__dot" aria-hidden="true"></span></div><span class="liq-price-marker__label">Now</span></div>`
    }
    ...
    function renderLiqMap(data: unknown) {
      ...
      type LiqLevel = { price: number; longLiqUsd?: number; shortLiqUsd?: number; totalUsd?: number }
      type FutData = {
        sym:string
        ticker:{lastPrice:string}
        liqMap?:{markPrice:number;levels:LiqLevel[];source?:string;eventCount?:number}
      }
      const d = data as FutData
      const sym = d.sym || futSym
      const mark=parseFloat(d.ticker?.lastPrice)||d.liqMap?.markPrice||lastLiqMarkBySym[sym]||0
      const levels=d.liqMap?.levels||[]
      if (!levels.length) return
      if (mark > 0) lastLiqMarkBySym[sym] = mark

      const maxUsd=Math.max(...levels.map(l=>l.totalUsd??((l.longLiqUsd||0)+(l.shortLiqUsd||0))),1)
      ...
      let html = sourceLabel
        ...
        : ''
      html += '<div class="liq-level-rows flex flex-col gap-[3px]">'
      reversed.forEach((l,i)=>{
        if(i===markIdx)html+=liqMarkerRowHtml(mark, markIdx)
        const total=l.totalUsd??((l.longLiqUsd||0)+(l.shortLiqUsd||0))
    ...
    async function refreshFut(force = false) {
      if (!force) {
        const cached = _cache.get(['futures', futSym].join(':'))
        if (cached && Date.now() - cached.ts < FUT_TTL) {
          renderFutPanels(cached.data)
          return
        }
      }
    ...
    function initFut() {
      ...
      if(pillsEl){
        ...
        pillsEl.addEventListener('click',e=>{
          ...
          if(!btn)return
          switchFutSym(btn.dataset.sym||'BTC')
        })
      }
      ...
      refreshFut()
      prefetchFutSymbols()
      setInterval(() => refreshFut(true),30_000)
    }
      ...
      <main className="max-w-[1440px] mx-auto px-6 pb-10 pt-6">
        ...
        <div className="fut-wrap">
          <div className="fut-left">
            ...
            <div id="liqmap-body" className="px-3.5 py-3 h-[280px] overflow-y-auto overflow-x-hidden">
              <div id="liqmap-levels" className="flex flex-col gap-[3px]">
