...
/** Root-relative in dev (Vite proxy); absolute when VITE_API_BASE_URL is set. */
...
// ── Dev-only sample data (see /?devlogin=1) ──────────────────────────────────
// With the dev bypass active there is no backend, so the gated dashboard pages
// would error out ("Connection Failed"). Return sample data for the LIST
// endpoints so the authenticated UI renders and can be reskinned locally. The
// per-trader detail panels (live positions/charts) stay empty unless a real
// backend is connected. import.meta.env.DEV is statically false in production
// builds, so this whole path is dead-code-eliminated and can never ship.
const devBypass = () =>
  import.meta.env.DEV && localStorage.getItem('eva_dev_bypass') === '1'

const MOCK_TRADERS: TraderInfo[] = [
  {
    trader_id: 'dev-1',
    trader_name: 'Alpha Scout',
    ai_model: 'gpt-4o',
    exchange_id: 'dev-ex',
    is_running: true,
    show_in_competition: true,
    strategy_name: 'Momentum Grid',
  },
  {
    trader_id: 'dev-2',
    trader_name: 'Mean Reverter',
    ai_model: 'claude-3-5-sonnet',
    exchange_id: 'dev-ex',
    is_running: false,
    show_in_competition: true,
    strategy_name: 'Range Scalper',
  },
]
...
function buildMockDecisions(limit: number = 6): DecisionRecord[] {
  const now = Date.now()
  const seed: Array<{
    symbol: string
    action: string
  ...
  }> = [
    {
      ...
      take_profit: 65800,
      reasoning:
        'Price reclaimed the 4h EMA200 with rising open interest and a bullish funding flip. Momentum favors continuation toward the prior swing high.',
    },
    {
      symbol: 'SOLUSDT',
      action: 'LONG',
      ...
      take_profit: 158.0,
      reasoning:
        'Higher-low structure on the 1h with expanding volume on the breakout candle. Risk-reward is favorable above the consolidation range.',
    },
    {
      symbol: 'ETHUSDT',
      action: 'SHORT',
      ...
      take_profit: 3240,
      reasoning:
        'Rejection at the descending trendline with bearish RSI divergence on the 4h. Reducing exposure as relative strength lags Bitcoin.',
    },
    {
      symbol: 'BNBUSDT',
      action: 'HOLD',
      ...
      take_profit: 0,
      reasoning:
        'Price is mid-range with neutral momentum and no clear edge. Waiting for a decisive break of the box before committing capital.',
    },
    {
      symbol: 'AVAXUSDT',
      action: 'LONG',
      ...
      take_profit: 32.4,
      reasoning:
        'Bounce off the weekly demand zone with improving netflow. Entering a partial position while keeping a tight invalidation.',
    },
    {
      symbol: 'DOGEUSDT',
      action: 'SELL',
      ...
      take_profit: 0,
      reasoning:
        'Closing the runner into resistance after a parabolic move. Booking realized gains as momentum cools and funding turns expensive.',
    },
  ]
  const n = Math.min(Math.max(limit, 1), seed.length)
  return seed.slice(0, n).map((d, i) => {
    const ts = new Date(now - (i + 1) * 27 * 60 * 1000).toISOString()
    const decision: DecisionAction = {
      action: d.action,
      symbol: d.symbol,
      quantity: d.leverage > 0 ? Number((1000 / d.price).toFixed(4)) : 0,
      leverage: d.leverage,
      price: d.price,
      stop_loss: d.stop_loss || undefined,
      take_profit: d.take_profit || undefined,
    ...
    return {
      timestamp: ts,
      cycle_number: 142 - i,
      system_prompt: 'You are an autonomous crypto futures trading agent.',
      input_prompt: `Market snapshot for ${d.symbol} across 1h and 4h timeframes.`,
      cot_trace: d.reasoning,
      ...
      account_state: {
        total_balance: 12840.27,
        available_balance: 8402.13,
        total_unrealized_profit: 227.77,
        position_count: 3,
        margin_used_pct: 34.0,
      },
      positions: [],
      candidate_coins: ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'BNBUSDT', 'AVAXUSDT'],
      decisions: [decision],
      execution_log: [
        `Evaluated ${d.symbol}`,
        `Action ${d.action} at confidence ${d.confidence}`,
      ],
      success: true,
    }
  })
...
export const api = {
  ...
  async getTraders(): Promise<TraderInfo[]> {
    if (devBypass()) return MOCK_TRADERS
  ...
  async getModelConfigs(): Promise<AIModel[]> {
    if (devBypass()) return MOCK_MODELS
  ...
  async getExchangeConfigs(): Promise<Exchange[]> {
    if (devBypass()) return MOCK_EXCHANGES
  ...
  async getStatus(traderId?: string): Promise<SystemStatus> {
    if (devBypass()) {
      const start = new Date(Date.now() - 1880 * 60 * 1000).toISOString()
      return {
        trader_id: traderId || 'dev-1',
        trader_name: 'Alpha Scout',
        ai_model: 'gpt-4o',
        is_running: true,
        start_time: start,
        runtime_minutes: 1880,
        call_count: 142,
  ...
  async getAccount(traderId?: string): Promise<AccountInfo> {
    if (devBypass()) {
      return {
        total_equity: 12840.27,
        wallet_balance: 12612.5,
        unrealized_profit: 227.77,
        available_balance: 8402.13,
        total_pnl: 2840.27,
        total_pnl_pct: 28.4,
        initial_balance: 10000,
      ...
      return [
        {
          ...
          quantity: 0.085,
          leverage: 5,
          unrealized_pnl: 164.87,
          unrealized_pnl_pct: 15.84,
          liquidation_price: 49210.4,
          margin_used: 1040.79,
        },
        {
          symbol: 'ETHUSDT',
          ...
          quantity: 0.62,
          leverage: 4,
          unrealized_pnl: -23.16,
          unrealized_pnl_pct: -4.37,
          liquidation_price: 4180.2,
          margin_used: 530.22,
        },
        {
          symbol: 'SOLUSDT',
          ...
          quantity: 9.4,
          leverage: 3,
          unrealized_pnl: 88.74,
          unrealized_pnl_pct: 19.91,
  ...
  async getDecisions(traderId?: string): Promise<DecisionRecord[]> {
    if (devBypass()) return buildMockDecisions(6)
    const url = traderId
  ...
  async getLatestDecisions(
    ...
    if (devBypass()) return buildMockDecisions(limit)
  ...
  async getStatistics(traderId?: string): Promise<Statistics> {
    if (devBypass()) {
      return {
        total_cycles: 142,
        successful_cycles: 138,
        failed_cycles: 4,
        total_open_positions: 51,
        total_close_positions: 48,
      }
    }
    const url = traderId
  ...
  async getEquityHistory(traderId?: string): Promise<any[]> {
    if (devBypass()) {
      ...
      for (let i = 0; i < points; i++) {
        ...
        // deterministic pseudo-noise so the curve looks organic but trends up
        const noise = Math.sin(i * 1.7) * 65 + Math.cos(i * 0.6) * 40
        const equity = Math.max(start - 200, base + noise)
        out.push({
          timestamp: new Date(
            now - (points - 1 - i) * 15 * 60 * 1000
          ).toISOString(),
          total_equity: Number(equity.toFixed(2)),
          pnl: Number((equity - start).toFixed(2)),
          cycle_number: i + 1,
        })
      }
      return out
