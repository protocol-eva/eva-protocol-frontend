...
export class HttpClient {
  ...
  constructor() {
    ...
    // In production, VITE_API_BASE_URL points to the backend (e.g. https://oko-api.flappy-sol.app)
    // In dev, requests go to '/' and are proxied by Vite to localhost:8080
  ...
  private async handleError(error: AxiosError): Promise<any> {
    // Dev-only bypass: there's no backend, so every gated request fails. Reject
    // quietly (no logout, no "Server Error"/"Network error" toast spam) and let
    // callers render empty states — so the dashboards can be reskinned locally.
    // Compiled out of production (import.meta.env.DEV === false).
    if (import.meta.env.DEV && localStorage.getItem('eva_dev_bypass') === '1') {
      return Promise.reject(error)
    }
    ...
    if (status === 401) {
      // Dev-only bypass: keep the mock session alive so authenticated UI can be
      // built against a backend that would otherwise reject the fake token.
      // Compiled out of production (import.meta.env.DEV === false).
      if (
        import.meta.env.DEV &&
        localStorage.getItem('eva_dev_bypass') === '1'
      ) {
        throw new Error('Unauthorized (ignored — dev bypass active)')
      }
